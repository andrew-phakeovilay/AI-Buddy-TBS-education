...
export function ChatPage() {
    ...
    return (
        <div className="flex h-full flex-col md:flex-row">
            <aside className={`${open?"fixed inset-1 left-0 z-40 w-64":"hidden"} md:static md:block  w-full md:w-80 bg-red-1 text-white`}>
                <div className={`${open?"absolute":"hidden"} md:hidden top-2 left-2`}
            ...
            <div className="relative flex flex-1 h-full w-full flex-col bg-red-2 text-red-12">
                ...
                <main className="relative flex flex-col h-full flex-1 overflow-hidden">
                    ...
                    <input id="chat" name="chat" type="text" className="absolute z-20 bottom-4 bg-red-3 p-2 w-9/12 sm:w-1/2 left-1/2 -translate-x-1/2 border border-red-6" placeholder="Poser une question" />
                </main>
            </div>
        </div>
    )
